...
public class 사칙연산 {
    ...
    public int solution(String arr[]) {
        ...
        for (int i = 0; i < arr.length; i += 2) {
            ...
            if(i == 0) list[i / 2] = num;
            else list[i / 2] = arr[i - 1].equals("+") ? num : -num;
        }

        for (int i = size - 1; i >= 0; i--) {
            for (int j = i; j < size; j++) {
                if (i == j) min[i][j] = max[i][j] = list[i];
                else {
    ...
    private void func(int a, int b, int x, int y, boolean value) {
        if (value && a < 0) {
    ...
    @Test
    void test() {
        assertEquals(solution(new String[]{"1", "-", "3", "+", "5", "-", "8"}), 1);
        assertEquals(solution(new String[]{"5", "-", "3", "+", "1", "+", "2", "-", "4"}), 3);
